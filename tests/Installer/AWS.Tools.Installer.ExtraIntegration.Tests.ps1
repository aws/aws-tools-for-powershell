...
AfterAll {
    ...
    foreach ($currentLegacyModule in $currentLegacyModules) {
        ...
        if (-not $wasOriginal) {
            ...
            try {
                Uninstall-Module -Name $currentLegacyModule.Name -Force -Confirm:$false -ErrorAction SilentlyContinue -WarningAction SilentlyContinue @script:InformationActionSplat
            }
            catch {
    ...
    if ($missingLegacyModules.Count -gt 0) {
        foreach ($missingLegacyModule in $missingLegacyModules) {
            ...
            try {
                ...
                Install-Module @installParams -WarningAction SilentlyContinue @script:InformationActionSplat
            }
            catch {
...
Describe -Skip:$SkipInstallerTests -Tag "Smoke", "High" "Installer - Extra Integration Tests" {
    ...
    Context "Default PSModulePath Behavior" {
        ...
        It "Should install and uninstall modules using default PSModulePath" {
            ...
            Install-AWSToolsModule -Name @('Common', 'CloudWatch') -SourceZipPath $script:CachedZipPath -Scope CurrentUser -Confirm:$false -Force -SkipIntegrityCheck -WarningAction SilentlyContinue @script:InformationActionSplat
        ...
        It "Should automatically include AWS.Tools.Common when specific modules are requested" {
            ...
            Install-AWSToolsModule -Name @('CloudWatch') -SourceZipPath $script:CachedZipPath -Scope CurrentUser -Confirm:$false -Force -SkipIntegrityCheck -WarningAction SilentlyContinue @script:InformationActionSplat
    ...
    Context "Legacy Module Cleanup" {
        ...
        It "Should remove legacy AWSPowerShell modules when using CleanUpLegacyModuleScope CurrentUser" {
            ...
            try {
                Install-Module -Name AWSPowerShell -Scope CurrentUser -Force -Confirm:$false -ErrorAction Stop -WarningAction SilentlyContinue @script:InformationActionSplat
            ...
            if ($PSVersionTable.PSEdition -eq 'Core') {
                try {
                    Install-Module -Name AWSPowerShell.NetCore -Scope CurrentUser -Force -Confirm:$false -ErrorAction Stop -WarningAction SilentlyContinue @script:InformationActionSplat
            ...
            Install-AWSToolsModule -Name @('Common') -SourceZipPath $script:CachedZipPath -Scope CurrentUser -CleanUpLegacyModuleScope CurrentUser -Cleanup -Confirm:$false -Force -SkipIntegrityCheck -WarningAction SilentlyContinue @script:InformationActionSplat
            ...
            Uninstall-AWSToolsModule -Confirm:$false -ErrorAction SilentlyContinue -WarningAction SilentlyContinue @script:InformationActionSplat
        }
    }
}
