...
InModuleScope AWS.Tools.Installer {
    ...
    Describe -Skip:$SkipInstallerTests -Tag "Smoke", "Low", "Medium", "High" "Installer - Format-ModuleTarget Unit Tests" {
        ...
        Context "AWS.Tools Module Type (Default)" {
            ...
            It "Should handle complex version numbers correctly" {
                ...
                # Assert - Use regex to match path regardless of platform (returns 3-part version)
                $result | Should -Match "1 AWS\.Tools modules \(versions: 4\.1\.853\) from .+"
            }
        }
        ...
        Context "AWS.Tools.Installer Module Type" {
            ...
            It "Should format target for AWS.Tools.Installer module type" {
                ...
                # Assert - Use regex to match path regardless of platform (returns 3-part version)
                $result | Should -Match "1 AWS\.Tools\.Installer modules \(versions: 1\.0\.2\) from .+"
            }
            ...
            It "Should format target for multiple AWS.Tools.Installer modules with different versions" {
                ...
                # Assert - Use regex to match path regardless of platform (both return same 3-part version)
                $result | Should -Match "2 AWS\.Tools\.Installer modules \(versions: 1\.0\.2\) from .+"
            }
        }
        ...
        Context "Version Grouping Logic" {
            ...
            It "Should show single version when many modules have same version (407 modules with version 4.1.964)" {
                # Arrange - Create 407 modules all with the same version 4.1.964
                $mockModules = @()
                $version = [Version]"4.1.964"
                # Create a representative sample of module names
                $moduleNames = @("Common", "EC2", "S3", "Lambda", "DynamoDB", "CloudWatch", "IAM", "RDS", "ECS", "SQS")
                for ($i = 1; $i -le 407; $i++) {
                    $moduleName = "AWS.Tools." + $moduleNames[$i % $moduleNames.Count] + $i
                    $mockModules += (New-MockModule -Name $moduleName -Version $version)
                }
                $targetPath = [System.IO.Path]::Combine($HOME, ".local", "share", "powershell", "Modules")
                
                # Act
                $result = Format-ModuleTarget -Modules $mockModules -TargetPath $targetPath
                
                # Assert
                # Should show "versions: 4.1.964" NOT "versions: 4.1.964 and 401 more"
                $result | Should -Be "407 AWS.Tools modules (versions: 4.1.964) from $targetPath"
                $result | Should -Not -Match "\d+ more"
            }
            
            It "Should show 'X more' only when there are more than 5 DISTINCT versions" {
                # Arrange - Create modules with 7 distinct versions (should trigger "X more")
                $mockModules = @(
                    (New-MockModule -Name "AWS.Tools.Common" -Version ([Version]"4.1.960")),
                    (New-MockModule -Name "AWS.Tools.EC2" -Version ([Version]"4.1.961")),
                    (New-MockModule -Name "AWS.Tools.S3" -Version ([Version]"4.1.962")),
                    (New-MockModule -Name "AWS.Tools.Lambda" -Version ([Version]"4.1.963")),
                    (New-MockModule -Name "AWS.Tools.DynamoDB" -Version ([Version]"4.1.964")),
                    (New-MockModule -Name "AWS.Tools.CloudWatch" -Version ([Version]"4.1.965")),
                    (New-MockModule -Name "AWS.Tools.IAM" -Version ([Version]"4.1.966"))
                )
                $targetPath = [System.IO.Path]::Combine([System.IO.Path]::GetDirectoryName($PSHome), "PowerShell", "Modules")
                ...
                # Assert
                # Should show first 5 versions and "and 2 more"
                $result | Should -Match "7 AWS\.Tools modules \(versions: .+ and 2 more\) from .+"
                # Verify it contains some of the expected versions
                $result | Should -Match "4\.1\.96[0-4]"
            }
            
            It "Should NOT show 'X more' when there are exactly 5 DISTINCT versions" {
                # Arrange - Create modules with exactly 5 distinct versions
                $mockModules = @(
                    (New-MockModule -Name "AWS.Tools.Common" -Version ([Version]"4.1.960")),
                    (New-MockModule -Name "AWS.Tools.EC2" -Version ([Version]"4.1.961")),
                    (New-MockModule -Name "AWS.Tools.S3" -Version ([Version]"4.1.962")),
                ...
                # Assert
                # Should show all 5 versions without "X more"
                $result | Should -Not -Match "\d+ more"
                $result | Should -Match "4\.1\.960"
                $result | Should -Match "4\.1\.961"
                $result | Should -Match "4\.1\.962"
                $result | Should -Match "4\.1\.963"
                $result | Should -Match "4\.1\.964"
            }
        }
    }
}
