...
Describe -Tag "Smoke" "Get-AWSCmdletName" {
    ...
    Context "Get-AWSCmdletName" {
        ...
        It "should support filtering by assembly name" {
            (Get-AWSCmdletName -Service 'SimpleSystemsManagement' | Where-Object { $_.CmdletName -eq 'Add-SSMResourceTag' }).Count | Should -BeGreaterThan 0
        }

        It "should support filtering by service name" {
            (Get-AWSCmdletName -Service 'AWS Systems Manager' | Where-Object { $_.CmdletName -eq 'Add-SSMResourceTag' }).Count | Should -BeGreaterThan 0
        }

        It "should support filtering by service prefix" {
            (Get-AWSCmdletName -Service 'SSM' | Where-Object { $_.CmdletName -eq 'Add-SSMResourceTag' }).Count | Should -BeGreaterThan 0
        }
        ...
        It "filtering by service prefix should use strict equality" {
            ...
            (Get-AWSCmdletName -Service 'S3C' | Where-Object { $_.CmdletName -eq 'Add-S3CPublicAccessBlock' }).Count | Should -BeGreaterThan 0
            (Get-AWSCmdletName -Service 'S3' | Where-Object { $_.CmdletName -eq 'Add-S3CPublicAccessBlock' }).Count | Should -BeExactly 0
        }
        ...
        It "should support filtering by cmdlet name" {
            (Get-AWSCmdletName -CmdletName Write-SSMComplianceItem).Count | Should -BeGreaterThan 0
        }
