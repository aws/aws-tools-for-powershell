...
namespace DesktopManifestPatcher
{
    class Program
    {
        ...
        static int Main(string[] args)
        {
            if (args.Length == 0)
            {
                Console.WriteLine("Patches the ExportedCmdlets member of the desktop edition .psd1 manifest ");
                ...
                Console.WriteLine();
                Console.WriteLine("Usage: DesktopManifestPatcher.exe module_folder");
                return 0;
            }
            ...
            try
            {
                ...
                var moduleAssembly = Assembly.LoadFrom(moduleAssemblyFilename);
                ...
                Type pscmdletType = amznBaseCmdletType.BaseType;
                while (!pscmdletType.FullName.Equals("System.Management.Automation.Cmdlet", StringComparison.Ordinal))
                {
                    pscmdletType = pscmdletType.BaseType;
                }

                var exportedCmdlets = moduleAssembly.GetTypes()
