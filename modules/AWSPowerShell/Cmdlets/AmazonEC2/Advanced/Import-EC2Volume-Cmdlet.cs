...
namespace Amazon.PowerShell.Cmdlets.EC2
{
    ...
    /// </para>
    /// </summary>
    [Cmdlet("Import", "EC2Volume", DefaultParameterSetName = ParamSet_Default)]
    [OutputType("Amazon.EC2.Model.ConversionTask", "System.String")]
    [AWSCmdlet("Uploads a virtual disk image file to Amazon S3 and optionally requests import conversion to an EBS volume.")]
    ...
    [AWSCmdletOutput("Amazon.EC2.Model.ConversionTask",
        ...
        "The service call response (type Amazon.EC2.Model.ImportVolumeResponse) can also be referenced from properties attached to the cmdlet entry in the $AWSHistory stack (if only one manifest or image was imported)"
    )]
    [AWSCmdletOutput("System.String",
        "Contains the Amazon S3 object key of the import manifest file that was created and uploaded by the cmdlet. "
            ...
            + "Use this value with the -ManifestFileKey parameter to request conversion to an EBS volume at a later time."
    )]
    public class ImportEC2VolumeCmdlet : EC2ImportCmdletsBase, IExecutor
    {
        ...
        /// </summary>
        [Parameter(ParameterSetName = ParamSet_Default, Mandatory = true)]
        ...
        protected override void ProcessRecord()
        {
            ...
            WriteWarning("This cmdlet has been deprecated and will be removed in a future release. Please use the replacement Import-EC2Snapshot cmdlet.");
        ...
        public object Execute(ExecutorContext context)
        {
            return ParameterSetName.Equals(ParamSet_FromManifest)
                ? ImportFromExistingArtifacts(context as ImportCmdletContextBase)
                : ImportFromNewArtifacts(context as ImportCmdletContextBase, UploadOnly.IsPresent);
        }
        ...
        private CmdletOutput ImportFromExistingArtifacts(ImportCmdletContextBase context)
        {
            ...
            try
            {
                ...
                foreach (var manifestFileKey in context.ManifestFileKey)
                {
                    ...
                    var diskImageDetail = importHelper.PopulateDiskImageDetail(manifestFileKey);
                    var volumeDetail = importHelper.PopulateVolumeDetail();

                    var request = new ImportVolumeRequest
                    {
                        AvailabilityZone = context.AvailabilityZone,
        ...
        private CmdletOutput ImportFromNewArtifacts(ImportCmdletContextBase context, bool uploadOnly)
        {
            ...
            try
            {
                ...
                try
                {
                    var manifestKey = importHelper.Upload(context.ImageFile,
                                                          context.FileFormat,
                                                          context.VolumeSize,
                                                          context.KeyPrefix,
                    ...
                    {
                        var response = importHelper.StartVolumeConversion(context.AvailabilityZone, context.Description);
                        output = new CmdletOutput
                        {
        ...
        public ExecutorContext CreateContext()
        {
            ...
            var context = new ImportCmdletContextBase
            {
                Region = this.Region,
...
#elif CORECLR
                return client.ImportVolumeAsync(request).GetAwaiter().GetResult();
#else
