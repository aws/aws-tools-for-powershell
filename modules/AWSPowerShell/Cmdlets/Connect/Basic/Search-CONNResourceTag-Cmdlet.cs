...
{
    /// <summary>
    /// Searches tags used in an Connect Customer instance using optional search criteria.<br/><br/>This cmdlet automatically pages all available results to the pipeline - parameters related to iteration are only needed if you want to manually control the paginated output. To disable autopagination, use -NoAutoIteration.
    /// </summary>
    ...
    [AWSCmdletOutput("Amazon.Connect.Model.TagSet or Amazon.Connect.Model.SearchResourceTagsResponse",
        ...
        "The service call response (type Amazon.Connect.Model.SearchResourceTagsResponse) can be returned by specifying '-Select *'."
    )]
    public partial class SearchCONNResourceTagCmdlet : AmazonConnectClientCmdlet, IExecutor
    {
        ...
        /// that lowercase and - are required.</para><para><b>Supported resource types</b></para><ul><li><para>agent</para></li><li><para>agent-state</para></li><li><para>routing-profile</para></li><li><para>standard-queue</para></li><li><para>security-profile</para></li><li><para>operating-hours</para></li><li><para>prompt</para></li><li><para>contact-flow</para></li><li><para>flow- module</para></li><li><para>transfer-destination (also known as quick connect)</para></li><li><para>metric</para></li></ul><para />
        ...
        private Amazon.Connect.Model.SearchResourceTagsResponse CallAWSServiceOperation(IAmazonConnect client, Amazon.Connect.Model.SearchResourceTagsRequest request)
        {
            ...
            try
            {
                return client.SearchResourceTagsAsync(request, _cancellationTokenSource.Token).GetAwaiter().GetResult();
            }
