...
namespace Amazon.PowerShell.Cmdlets.CONN
{
    ...
    public partial class GetCONNCurrentMetricDataCmdlet : AmazonConnectClientCmdlet, IExecutor
    {
        ...
        /// queue</a></para></dd><dt>CONTACTS_SCHEDULED</dt><dd><para>Unit: COUNT</para><para>Name in real-time metrics report: <a href="https://docs.aws.amazon.com/connect/latest/adminguide/metrics-definitions.html#scheduled-real-time">Scheduled</a></para></dd><dt>ESTIMATED_WAIT_TIME</dt><dd><para>Unit: SECONDS</para><para>This metric supports filter and grouping combination only used for core routing purpose.
        /// Valid filter and grouping use cases: </para><ul><li><para>Filter by a list of [Queues] and a list of [Channels], group by [“QUEUE”, “CHANNEL”]</para></li><li><para>Filter by a singleton list of [Queue], a singleton list of [Channel], a list of [RoutingStepExpression],
        /// group by [“ROUTING_STEP_EXPRESSION”].</para></li></ul></dd><dt>OLDEST_CONTACT_AGE</dt><dd><para>Unit: SECONDS</para><para>When you use groupings, Unit says SECONDS and the Value is returned in SECONDS. </para><para>When you do not use groupings, Unit says SECONDS but the Value is returned in MILLISECONDS.
        ...
        private Amazon.Connect.Model.GetCurrentMetricDataResponse CallAWSServiceOperation(IAmazonConnect client, Amazon.Connect.Model.GetCurrentMetricDataRequest request)
        {
            ...
            try
            {
                return client.GetCurrentMetricDataAsync(request, _cancellationTokenSource.Token).GetAwaiter().GetResult();
            }
