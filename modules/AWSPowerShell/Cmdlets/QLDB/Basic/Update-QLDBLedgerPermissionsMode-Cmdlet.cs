...
namespace Amazon.PowerShell.Cmdlets.QLDB
{
    ...
    public partial class UpdateQLDBLedgerPermissionsModeCmdlet : AmazonQLDBClientCmdlet, IExecutor
    {
        ...
        /// values:</para><ul><li><para><code>ALLOW_ALL</code>: A legacy permissions mode that enables access control with
        /// API-level granularity for ledgers.</para><para>This mode allows users who have <code>SendCommand</code> permissions for this ledger
        ...
        /// that you create for the ledger.</para></li><li><para><code>STANDARD</code>: (<i>Recommended</i>) A permissions mode that enables access
        ...
        /// API permissions for the ledger.</para></li></ul><note><para>We strongly recommend using the <code>STANDARD</code> permissions mode to maximize
        ...
        private Amazon.QLDB.Model.UpdateLedgerPermissionsModeResponse CallAWSServiceOperation(IAmazonQLDB client, Amazon.QLDB.Model.UpdateLedgerPermissionsModeRequest request)
        {
            ...
            {
                #if DESKTOP
                return client.UpdateLedgerPermissionsMode(request);
                #elif CORECLR
                return client.UpdateLedgerPermissionsModeAsync(request).GetAwaiter().GetResult();
                #else
