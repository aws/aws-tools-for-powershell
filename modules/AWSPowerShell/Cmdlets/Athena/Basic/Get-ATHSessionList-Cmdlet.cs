...
namespace Amazon.PowerShell.Cmdlets.ATH
{
    ...
    /// listed first; older sessions are listed later.<br/><br/>This cmdlet automatically pages all available results to the pipeline - parameters related to iteration are only needed if you want to manually control the paginated output. To disable autopagination, use -NoAutoIteration.
    /// </summary>
    ...
    [AWSCmdletOutput("Amazon.Athena.Model.SessionSummary or Amazon.Athena.Model.ListSessionsResponse",
        ...
        "The service call response (type Amazon.Athena.Model.ListSessionsResponse) can also be referenced from properties attached to the cmdlet entry in the $AWSHistory stack."
    )]
    public partial class GetATHSessionListCmdlet : AmazonAthenaClientCmdlet, IExecutor
    {
        ...
        /// <summary>
        /// <para>
        /// <para>A filter for a specific session state. A description of each state follows.</para><para><code>CREATING</code> - The session is being started, including acquiring resources.</para><para><code>CREATED</code> - The session has been started.</para><para><code>IDLE</code> - The session is able to accept a calculation.</para><para><code>BUSY</code> - The session is processing another task and is unable to accept
        /// a calculation.</para><para><code>TERMINATING</code> - The session is in the process of shutting down.</para><para><code>TERMINATED</code> - The session and its resources are no longer running.</para><para><code>DEGRADED</code> - The session has no healthy coordinators.</para><para><code>FAILED</code> - Due to a failure, the session and its resources are no longer
        /// running.</para>
        /// </para>
        /// </summary>
