...
namespace Amazon.PowerShell.Cmdlets.GMLS
{
    ...
    public partial class NewGMLSApplicationCmdlet : AmazonGameLiftStreamsClientCmdlet, IExecutor
    {
        ...
        /// up to 10 log paths. Each individual log file cannot exceed 50 MB in size.</para><para>Each path can be a directory or an exact file path. When you specify a directory,
        ...
        /// exact file path. The copy operation is not performed recursively in subfolders.</para><para>The following path variables are recognized when they appear as the first component
        /// of a path: <c>%USERPROFILE%</c> (Windows and Proton), <c>$HOME</c> or <c>~</c> (Linux).
        /// Use a path variable when your application writes logs outside of the application directory.</para><para>Amazon GameLift Streams uploads designated log files to the Amazon S3 bucket that
        ...
        /// <summary>
        /// <para>
        /// <para>The relative path and file name of the executable file that Amazon GameLift Streams
