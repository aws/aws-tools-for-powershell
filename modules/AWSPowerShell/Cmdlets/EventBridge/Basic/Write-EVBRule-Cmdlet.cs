...
namespace Amazon.PowerShell.Cmdlets.EVB
{
    ...
    public partial class WriteEVBRuleCmdlet : AmazonEventBridgeClientCmdlet, IExecutor
    {
        ...
        /// <summary>
        /// <para>
        /// <para>The state of the rule.</para><para>Valid values include:</para><ul><li><para><code>DISABLED</code>: The rule is disabled. EventBridge does not match any events
        /// against the rule.</para></li><li><para><code>ENABLED</code>: The rule is enabled. EventBridge matches events against the
        /// rule, <i>except</i> for Amazon Web Services management events delivered through CloudTrail.</para></li><li><para><code>ENABLED_WITH_ALL_CLOUDTRAIL_MANAGEMENT_EVENTS</code>: The rule is enabled for
        /// all events, including Amazon Web Services management events delivered through CloudTrail.</para><para>Management events provide visibility into management operations that are performed
        ...
        /// User Guide</i>.</para><para>This value is only valid for rules on the <a href="https://docs.aws.amazon.com/eventbridge/latest/userguide/eb-what-is-how-it-works-concepts.html#eb-bus-concepts-buses">default</a>
