...
namespace Amazon.PowerShell.Cmdlets.ECS
{
    ...
    public partial class RegisterECSTaskDefinitionCmdlet : AmazonECSClientCmdlet, IExecutor
    {
        ...
        /// and <code>awsvpc</code> network modes offer the highest networking performance for
        /// containers because they use the EC2 network stack instead of the virtualized network
        /// stack provided by the <code>bridge</code> mode.</para><para>With the <code>host</code> and <code>awsvpc</code> network modes, exposed container
        /// ports are mapped directly to the corresponding host port (for the <code>host</code>
        /// network mode) or the attached ENI port (for the <code>awsvpc</code> network mode),
        /// so you cannot take advantage of dynamic host port mappings. </para><para>If the network mode is <code>awsvpc</code>, the task is allocated an Elastic Network
        ...
        /// Networking</a> in the <i>Amazon EC2 Container Service Developer Guide</i>.</para><para>If the network mode is <code>host</code>, you can not run multiple instantiations
        /// of the same task on a single container instance when port mappings are used.</para><para>For more information, see <a href="https://docs.docker.com/engine/reference/run/#network-settings">Network
