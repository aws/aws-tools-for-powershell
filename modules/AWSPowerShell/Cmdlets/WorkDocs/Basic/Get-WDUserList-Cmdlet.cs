...
namespace Amazon.PowerShell.Cmdlets.WD
{
    ...
    [AWSCmdletOutput("Amazon.WorkDocs.Model.User or Amazon.WorkDocs.Model.DescribeUsersResponse",
        ...
        "The service call response (type Amazon.WorkDocs.Model.DescribeUsersResponse) can be returned by specifying '-Select *'."
    )]
    public partial class GetWDUserListCmdlet : AmazonWorkDocsClientCmdlet, IExecutor
    {
        ...
        /// <summary>
        /// <para>
        /// <para>A query to filter users by user name. Remember the following about the <c>Userids</c>
        /// and <c>Query</c> parameters:</para><ul><li><para>If you don't use either parameter, the API returns a paginated list of all users on
        /// the site.</para></li><li><para>If you use both parameters, the API ignores the <c>Query</c> parameter.</para></li><li><para>The <c>Userid</c> parameter only returns user names that match a corresponding user
        /// ID.</para></li><li><para>The <c>Query</c> parameter runs a "prefix" search for users by the <c>GivenName</c>,
        ...
        /// matches all characters. For example, querying on <c>Ma J</c> only returns Mateo Jackson.</para></li></ul>
        /// </para>
        /// </summary>
        [System.Management.Automation.Parameter(ValueFromPipelineByPropertyName = true)]
