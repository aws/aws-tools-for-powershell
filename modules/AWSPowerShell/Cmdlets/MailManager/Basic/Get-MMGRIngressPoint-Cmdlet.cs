...
namespace Amazon.PowerShell.Cmdlets.MMGR
{
    ...
    public partial class GetMMGRIngressPointCmdlet : AmazonMailManagerClientCmdlet, IExecutor
    {
        ...
        private Amazon.MailManager.Model.GetIngressPointResponse CallAWSServiceOperation(IAmazonMailManager client, Amazon.MailManager.Model.GetIngressPointRequest request)
        {
            Utils.Common.WriteVerboseEndpointMessage(this, client.Config, "Amazon SES Mail Manager", "GetIngressPoint");
            try
            {
                #if DESKTOP
                return client.GetIngressPoint(request);
                #elif CORECLR
                return client.GetIngressPointAsync(request).GetAwaiter().GetResult();
                #else
