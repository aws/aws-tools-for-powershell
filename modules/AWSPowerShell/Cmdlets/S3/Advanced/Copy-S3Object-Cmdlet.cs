...
namespace Amazon.PowerShell.Cmdlets.S3
{
    ...
    [AWSCmdletOutput("Amazon.S3.Model.S3Object or System.IO.FileInfo or System.IO.DirectoryInfo or Amazon.S3.Transfer.TransferUtilityDownloadResponse or Amazon.S3.Transfer.TransferUtilityDownloadDirectoryResponse",
        "When copying an object to another object in S3 the cmdlet returns an Amazon.S3.Model.S3Object referencing the new object."
            ...
            + " When -UseMultipartDownload is specified for S3-to-local download operations, returns TransferUtilityDownloadResponse (single file) or TransferUtilityDownloadDirectoryResponse (directory) with download metadata."
    )]
    public class CopyS3ObjectCmdlet : AmazonS3ClientCmdlet, IExecutor
    {
        ...
        /// </para>
        /// <para>This parameter only applies to S3-to-local download operations, not S3-to-S3 copy operations.</para>
        /// </summary>
        [Parameter(ParameterSetName = CopySingleObjectToLocalFile, ValueFromPipelineByPropertyName = true)]
        [Parameter(ParameterSetName = CopySingleObjectToLocalFolder, ValueFromPipelineByPropertyName = true)]
        [Parameter(ParameterSetName = CopyMultipleObjectsToLocalFolder, ValueFromPipelineByPropertyName = true)]
        ...
        private object CopyS3ObjectToLocalFile(ExecutorContext context)
        {
            ...
            using (var tu = CreateDownloadTransferUtility(cmdletContext))
            {
                ...
                if (cmdletContext.UseMultipartDownload)
                {
                    ...
                    {
                        WriteVerboseDownloadResponse(response, cmdletContext.LocalFile);
        ...
        private object CopyS3ObjectsToLocalFolder(ExecutorContext context)
        {
            ...
            using (var tu = CreateDownloadTransferUtility(cmdletContext))
            {
                ...
                if (cmdletContext.UseMultipartDownload)
                {
                    ...
                    if (output.ErrorResponse == null)
                    {
                        WriteVerboseDirectoryDownloadResponse(dirResponse, cmdletContext.LocalFolder);
        ...
        private TransferUtility CreateDownloadTransferUtility(CmdletContext cmdletContext)
        {
            if (cmdletContext.UseMultipartDownload)
            {
