...
namespace Amazon.PowerShell.Cmdlets.S3
{
    ...
    public partial class GetS3VersionCmdlet : AmazonS3ClientCmdlet, IExecutor
    {
        ...
        /// <br/>'KeyMarker' is only returned by the cmdlet when '-Select *' is specified. In order to manually control output pagination, set '-KeyMarker' to null for the first call then set the 'KeyMarker' using the same property output from the previous call for subsequent calls.
        /// </para>
        /// </summary>
        [System.Management.Automation.Parameter(ValueFromPipelineByPropertyName = true)]
        [Alias("NextToken")]
        ...
        public object Execute(ExecutorContext context)
        {
            ...
            var _nextToken = cmdletContext.KeyMarker;
            var _userControllingPaging = this.NoAutoIteration.IsPresent || ParameterWasBound(nameof(this.KeyMarker));
            var _shouldAutoIterate = !(SessionState.PSVariable.GetValue("AWSPowerShell_AutoIteration_Mode")?.ToString() == "v4");
            
            var client = Client ?? CreateClient(_CurrentCredentials, _RegionEndpoint);
            do
            {
                request.KeyMarker = _nextToken;
                ...
                try
                {
                    ...
                    _nextToken = response.NextKeyMarker;
                }
                catch (Exception e)
                {
