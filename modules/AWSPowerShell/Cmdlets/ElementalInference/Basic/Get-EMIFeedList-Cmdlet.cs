...
namespace Amazon.PowerShell.Cmdlets.EMI
{
    ...
    public partial class GetEMIFeedListCmdlet : AmazonElementalInferenceClientCmdlet, IExecutor
    {
        ...
        /// also returns a NextToken value that you can use to fetch the next batch of results.)</para><para>The service might return fewer results than the MaxResults value. If MaxResults is
        ...
        /// <summary>
        /// <para>
        /// <para>The token that identifies the batch of results that you want to see.</para><para>For example, you submit a ListBridges request with MaxResults set at 5. The service
        ...
        private Amazon.ElementalInference.Model.ListFeedsResponse CallAWSServiceOperation(IAmazonElementalInference client, Amazon.ElementalInference.Model.ListFeedsRequest request)
        {
            ...
            {
                #if DESKTOP
                return client.ListFeeds(request);
                #elif CORECLR
                return client.ListFeedsAsync(request).GetAwaiter().GetResult();
                #else
