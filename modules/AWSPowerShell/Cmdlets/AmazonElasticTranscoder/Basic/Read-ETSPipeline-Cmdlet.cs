...
namespace Amazon.PowerShell.Cmdlets.ETS
{
    ...
    /// </summary>
    [Cmdlet("Read", "ETSPipeline", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType("Amazon.ElasticTranscoder.Model.Pipeline")]
    [AWSCmdlet("Calls the Amazon Elastic Transcoder ReadPipeline API operation.", Operation = new[] {"ReadPipeline"})]
    ...
    public partial class ReadETSPipelineCmdlet : AmazonElasticTranscoderClientCmdlet, IExecutor
    {
        ...
        protected override void ProcessRecord()
        {
            ...
            if (!ConfirmShouldProceed(this.Force.IsPresent, resourceIdentifiersText, "Read-ETSPipeline (ReadPipeline)"))
            {
                return;
            }
        ...
        private Amazon.ElasticTranscoder.Model.ReadPipelineResponse CallAWSServiceOperation(IAmazonElasticTranscoder client, Amazon.ElasticTranscoder.Model.ReadPipelineRequest request)
        {
            Utils.Common.WriteVerboseEndpointMessage(this, client.Config, "Amazon Elastic Transcoder", "ReadPipeline");
            try
            {
                #if DESKTOP
                return client.ReadPipeline(request);
                #elif CORECLR
                return client.ReadPipelineAsync(request).GetAwaiter().GetResult();
                #else
