...
namespace Amazon.PowerShell.Cmdlets.SEC
{
    ...
    [AWSCmdlet("Calls the AWS Secrets Manager CreateSecret API operation.", Operation = new[] {"CreateSecret"})]
    [AWSCmdletOutput("Amazon.SecretsManager.Model.CreateSecretResponse",
        "This cmdlet returns a Amazon.SecretsManager.Model.CreateSecretResponse object containing multiple properties. The object can also be referenced from properties attached to the cmdlet entry in the $AWSHistory stack."
    )]
    public partial class NewSECSecretCmdlet : AmazonSecretsManagerClientCmdlet, IExecutor
    {
        ...
        /// tool to pass the contents of the file as a parameter.</para><para>Either <code>SecretString</code>, <code>SecretBinary</code>, or both must have a value.
        /// They cannot both be empty.</para><para>This <code>SecretBinary</code> value is stored separately from the <code>SecretString</code>,
        /// but the two parameters jointly share a maximum size limit.</para><para>This parameter is not available using the Secrets Manager console. It can be accessed
        ...
        /// of the secret.</para><para>Either <code>SecretString</code>, <code>SecretBinary</code>, or both must have a value.
        /// They cannot both be empty.</para><para>This string value is stored separately from the <code>SecretBinary</code>, but the
        /// two parameters jointly share a maximum size limit.</para><para>If you create a secret by using the Secrets Manager console then Secrets Manager puts
        ...
        /// JSON for Parameters</a> in the <i>AWS CLI User Guide</i>. For example:</para><para><code>[{"Key":"username","Value":"bob"},{"Key":"password","Value":"abc123xyz456"}]</code></para><para>If your command-line tool or SDK requires quotation marks around the parameter, you
        ...
        private Amazon.SecretsManager.Model.CreateSecretResponse CallAWSServiceOperation(IAmazonSecretsManager client, Amazon.SecretsManager.Model.CreateSecretRequest request)
        {
            ...
            try
            {
                ...
                var task = client.CreateSecretAsync(request);
                return task.Result;
                #else
