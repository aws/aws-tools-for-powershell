...
namespace Amazon.PowerShell.Cmdlets.QS
{
    ...
    public partial class NewQSKnowledgeBaseCmdlet : AmazonQuickSightClientCmdlet, IExecutor
    {
        ...
        /// <summary>
        /// <para>
        /// <para>Specifies whether ACLs are enabled for the knowledge base.</para><para>This setting works together with the data source connector's ACL crawling. To enforce
        ...
        /// two settings does not produce a fully ACL-enforced knowledge base.</para>
        /// </para>
        /// </summary>
        [System.Management.Automation.Parameter(ValueFromPipelineByPropertyName = true)]
        ...
        /// <summary>
        /// <para>
        /// <para>The Amazon Resource Name (ARN) of the Amazon QuickSight user or group to set as the
        /// primary owner of the knowledge base. The specified principal is always granted owner
        /// access, regardless of what is specified in the <c>Permissions</c> field.</para><para>This must be an Amazon QuickSight principal ARN, not an IAM user or role ARN. The
        ...
        /// without an owner, even when you call the operation as an Amazon QuickSight user.</para><para>When you call <c>CreateKnowledgeBase</c> as an IAM user or an assumed IAM role, specify
        ...
        /// a primary owner is recommended.</para>
        /// </para>
        /// </summary>
        [System.Management.Automation.Parameter(ValueFromPipelineByPropertyName = true)]
        ...
        /// <summary>
        /// <para>
        /// <para>The connector configuration for the knowledge base data source. The structure depends
        /// on the connector type of the data source referenced by <c>DataSourceArn</c>.</para><para>The template must be a JSON object. All connector types share the following top-level
        /// keys. The value of <c>type</c> and the contents of <c>connectionConfiguration</c>
        /// vary by connector type.</para><ul><li><para><c>type</c> – (Required) The connector type of the data source. This value identifies
        ...
        /// see the connector-specific list that follows.</para></li><li><para><c>connectionConfiguration</c> – (Required) The connection details for the data source.
        /// The keys in this object vary by connector type; see the connector-specific list that
        /// follows.</para></li><li><para><c>filterConfiguration</c> – (Optional) Rules that determine which content is crawled,
        /// such as inclusion and exclusion prefixes, patterns, or file-size limits.</para></li><li><para><c>accessControlConfiguration</c> – (Optional) Document-level access control (ACL)
        /// settings. Supported by all connector types except Web Crawler (<c>WEBCRAWLERV3</c>).
        /// The available fields depend on the connector type.</para></li><li><para><c>deletionProtectionConfiguration</c> – (Optional) Deletion-protection settings,
        /// supported by all connector types. Contains <c>enableDeletionProtection</c> (Boolean)
        /// and <c>deletionProtectionThreshold</c> (String; a value from 1 to 100).</para></li></ul><para>The following list describes the valid <c>type</c> value, the <c>connectionConfiguration</c>
        /// contents, and any connector-specific fields for each connector type:</para><ul><li><para><b>Amazon S3</b> (<c>type</c>: <c>S3V2</c>) – The <c>type</c> value must be <c>S3V2</c>.
        /// <c>connectionConfiguration</c> is required and contains:</para><ul><li><para><c>bucketName</c> – (Required) The name of the Amazon S3 bucket to crawl. Type: String.
        /// Length: 3–63 characters. Pattern: <c>^[a-z0-9][.\-a-z0-9]{1,61}[a-z0-9]$</c>.</para></li><li><para><c>bucketOwnerAccountId</c> – (Required) The ID of the AWS account that owns the
        /// bucket. Type: String. Pattern: <c>^\d{12}$</c>.</para></li></ul><para>Amazon S3 supports the following optional <c>filterConfiguration</c> fields:</para><ul><li><para><c>inclusionPrefixes</c> or <c>exclusionPrefixes</c> – Amazon S3 key prefixes to
        /// include or exclude. Type: Array of String. Up to 350 items, each 1–1,024 characters.</para></li><li><para><c>inclusionPatterns</c> or <c>exclusionPatterns</c> – Patterns to include or exclude
        /// objects. Type: Array of String. Up to 350 items, each 1–1,024 characters.</para></li><li><para><c>maxFileSizeInMegaBytes</c> – The maximum size, in MB, of a file to ingest. Type:
        /// String. Pattern: <c>^\d+$</c>.</para></li></ul><para>For Amazon S3, <c>accessControlConfiguration</c> supports the following fields:</para><ul><li><para><c>crawlAcl</c> – Specifies whether the connector crawls and enforces document access
        ...
        /// metadata files.</para></li><li><para><c>aclConfigurationFilePath</c> – The Amazon S3 URI of the global ACL configuration
        /// file. Type: String. Length: 1–1,024 characters. Optional. If you don't provide a global
        /// ACL configuration file, define ACLs in per-document metadata files.</para></li><li><para><c>defaultAccessType</c> – The access behavior applied to Amazon S3 prefixes that
        /// are not listed in the ACL configuration. Type: String. The only supported value is
        /// <c>ALLOW</c>.</para></li></ul><para><c>metadataFilesPrefix</c> – (Optional) The Amazon S3 prefix under which per-document
        ...
        /// String. Length: 1–1,024 characters.</para></li><li><para><b>Google Drive</b> (<c>type</c>: <c>GOOGLEDRIVEV3</c>) – Requires <c>connectionConfiguration</c>
        /// with <c>authType</c> set to <c>SERVICE_ACCOUNT</c>. Supports <c>dataEntityConfiguration</c>
        /// with <c>crawlMyDrive</c>, <c>crawlSharedWithMe</c>, and <c>crawlSharedDrives</c>.</para></li><li><para><b>OneDrive</b> (<c>type</c>: <c>ONEDRIVEV3</c>) – Requires <c>authType</c> at the
        ...
        /// <c>crawlPersonalDrives</c> and <c>crawlSharedWithMe</c>.</para></li><li><para><b>SharePoint</b> (<c>type</c>: <c>SHAREPOINTV3</c>) – Requires <c>connectionConfiguration</c>
        /// with <c>tenantId</c> in UUID format. Supports <c>dataEntityConfiguration</c> with
        /// <c>siteUrls</c>, <c>crawlFiles</c>, and <c>crawlPages</c>.</para></li><li><para><b>Web Crawler</b> (<c>type</c>: <c>WEBCRAWLERV3</c>) – Requires <c>connectionConfiguration</c>
        ...
        /// <c>authType</c>: <c>NO_AUTH</c>, <c>BASIC_AUTH</c>, <c>FORM</c>, <c>SAML</c>.</para></li></ul><para><b>Enabling document-level access control for Amazon S3</b></para><para>For an Amazon S3 (<c>S3V2</c>) knowledge base, document-level access control is governed
        /// by two settings that must both be enabled:</para><ol><li><para>In this template, set <c>accessControlConfiguration.crawlAcl</c> to <c>true</c>. Define
        ...
        /// in the ACL file, you can also set <c>accessControlConfiguration.defaultAccessType</c>.</para></li><li><para>In the <c>CreateKnowledgeBase</c> or <c>UpdateKnowledgeBase</c> request, set the top-level
        /// <c>AccessControlConfiguration.isACLEnabled</c> to <c>true</c>.</para></li></ol>
        /// </para>
        /// </summary>
        [System.Management.Automation.Parameter(ValueFromPipelineByPropertyName = true)]
