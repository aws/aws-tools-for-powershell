...
namespace Amazon.PowerShell.Cmdlets.ASA
{
    ...
    public partial class AddASACommunicationToCaseCmdlet : AmazonAWSSupportClientCmdlet, IExecutor
    {
        ...
        {
            Utils.Common.WriteVerboseEndpointMessage(this, client.Config, "AWS Support API", "AddCommunicationToCase");
            try
            {
                #if DESKTOP
                return client.AddCommunicationToCase(request);
                #elif CORECLR
                return client.AddCommunicationToCaseAsync(request).GetAwaiter().GetResult();
                #else
