...
namespace Amazon.PowerShell.Cmdlets.QLDBS
{
    ...
    public partial class SendQLDBSCommandCmdlet : AmazonQLDBSessionClientCmdlet, IExecutor
    {
        ...
        /// computed by QLDB.</para><para>The purpose of the <c>CommitDigest</c> parameter is to ensure that QLDB commits a
        /// transaction if and only if the server has processed the exact set of statements sent
        /// by the client, in the same order that client sent them, and with no duplicates.</para>
        /// </para>
        ...
        /// the life of the session.</para><para>To obtain a session token, run the <c>StartSession</c> command. This <c>SessionToken</c>
