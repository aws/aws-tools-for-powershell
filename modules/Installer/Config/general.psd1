@{
    # Maximum number of versions to display in ShouldProcess messages before truncating
    MaxVersionsToShow = 5
    ...
    Modules = @{
        "AWS.Tools" = @{
            ...
            CloudFrontUrls = @{
                ...
                # Preview version URL pattern (uses full semver string including prerelease tag)
                ZipDownloadUrlPreviewPattern = "{baseUrl}/{path}/releases/AWS.Tools.{semver}.zip"
                # Latest preview URL pattern (for -Prerelease without specific version)
                ZipDownloadUrlLatestPreviewPattern = "{baseUrl}/{path}/v{majorVersion}/latest/AWS.Tools-preview.zip"
            }
        ...
        "AWS.Tools.Installer" = @{
            ...
            CloudFrontUrls = @{
                ...
                # Preview version URL pattern (uses full semver string including prerelease tag)
                ZipDownloadUrlPreviewPattern = "{baseUrl}/{path}/releases/AWS.Tools.Installer.{semver}.zip"
                # Latest preview URL pattern (for -Prerelease without specific version)
                ZipDownloadUrlLatestPreviewPattern = "{baseUrl}/{path}/v{majorVersion}/latest/AWS.Tools.Installer-preview.zip"
            }
            
            # Module metadata
            ModulePrefix = "AWS.Tools.Installer"
            ZipFileName = "AWS.Tools.Installer.zip"
            DefaultMajorVersion = "2"  # Installer v2.x uses v2 path
