...
function Get-AWSToolsModule {
    ...
    Process {
        ...
        if ($Path) {
            # When a specific path is provided, ONLY search in that path
            # This is important to avoid finding modules from other locations (e.g., development paths)
        ...
        try {
            ...
            # Additional filter: ensure modules are actually in the specified path
            # This handles edge cases where PSModulePath manipulation doesn't fully restrict the search
            if ($Path -and $installedAwsToolsModules) {
        ...
        if ($installedAwsToolsModules -and ($PSVersionTable.PSVersion.Major -lt $script:Config.versions.PowerShell.PowerShellCoreMinimumMajorVersion -or $IsWindows)) {
            $installedAwsToolsModules = $installedAwsToolsModules | 
                Where-Object {
                    ...
                    $validSubject = $signature.SignerCertificate -and (
