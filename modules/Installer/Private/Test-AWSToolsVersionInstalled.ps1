...
function Test-AWSToolsVersionInstalled {
    ...
    Process {
        ...
        if ($script:VersionCache -and $script:VersionCache.ContainsKey($cacheKey)) {
            ...
            if ($Version) {
                # Check if this is a "latest preview" marker
                $isLatestPreview = $false
                if ($Version.PSObject.Properties['IsLatestPreview']) {
                    $isLatestPreview = $Version.IsLatestPreview
                }
                
                if ($isLatestPreview) {
                    # Handle latest preview version (from -Prerelease without specific version)
                    $majorVersion = $Version.Major
                    
                    $zipUrl = $moduleConfig.CloudFrontUrls.ZipDownloadUrlLatestPreviewPattern `
                        -replace '\{baseUrl\}', $baseUrl `
                        -replace '\{path\}', $path `
                        -replace '\{majorVersion\}', $majorVersion
                }
                # Check if this is a preview/prerelease version (semver object with IsPreRelease property)
                elseif ($Version.PSObject.Properties['IsPreRelease'] -and $Version.IsPreRelease) {
                    # Handle preview/prerelease versions (semver PSCustomObject)
                    $versionString = $Version.SemVerString
                    $majorVersion = $Version.Major
                    
                    # Use the preview URL pattern if available, otherwise fall back to version pattern
                    ...
                    $zipUrl = $urlPattern `
                        ...
                        -replace '\{semver\}', $versionString
                }
                else {
                    # Handle standard versions - could be [Version] object or string
                    $versionObj = $Version
                    if ($Version -is [string]) {
                        # Check if it's a semver string (contains prerelease tag)
                        if ($Version -match '^(\d+)\.(\d+)\.(\d+)-.+$') {
                            # This is a semver string - extract base version
                            $versionObj = [Version]::new([int]$Matches[1], [int]$Matches[2], [int]$Matches[3])
                            $versionString = $Version  # Use the full semver string
                            $majorVersion = [int]$Matches[1]
                            
                            # Use the preview URL pattern
                            $urlPattern = $moduleConfig.CloudFrontUrls.ZipDownloadUrlPreviewPattern
                            if (-not $urlPattern) {
                                $urlPattern = $moduleConfig.CloudFrontUrls.ZipDownloadUrlVersionPattern
                            }
                            ...
                            # Check if it's a wildcard pattern - skip version check for wildcards
                            if ($Version -match '^\d+\.\*$') {
                                return $false
                            }
                            ...
                            $versionString = "$($versionObj.Major).$($versionObj.Minor).$($versionObj.Build)"
        ...
        if ($Name -eq 'AWS.Tools') {
            ...
            $installedModule = Get-InstalledAWSToolsModule @params |
