    ...
    Process {
        ...
        if ($Version) {
            ...
            if ($isLatestPreview) {
                ...
                Write-Verbose ("[$($MyInvocation.MyCommand)] Using CloudFront source for latest " +
                    "preview in major version: $majorVersion")
                
                $zipSource = $moduleConfig.CloudFrontUrls.ZipDownloadUrlLatestPreviewPattern `
            ...
            elseif ($Version.PSObject.Properties['IsPreRelease'] -and $Version.IsPreRelease) {
                ...
                $semverString = $Version.SemVerString
                $majorVersion = $Version.Major
                Write-Verbose ("[$($MyInvocation.MyCommand)] Using CloudFront source for preview " +
                ...
                $zipSource = $urlPattern `
                    -replace '\{baseUrl\}', $baseUrl `
                    -replace '\{path\}', $cloudFrontPath `
                    -replace '\{majorVersion\}', $majorVersion `
                    -replace '\{version\}', $semverString `
                    -replace '\{semver\}', $semverString
            }
            else {
                # Handle standard versions - could be [Version] object or string
                # Convert to [Version] if it's a string
                $versionObj = $Version
                if ($Version -is [string]) {
                    # Check if it's a semver string (contains prerelease tag)
                    if ($Version -match '^(\d+)\.(\d+)\.(\d+)-.+$') {
                        # This is a semver string - use preview URL pattern
                        $semverString = $Version
                        $majorVersion = [int]$Matches[1]
                        Write-Verbose ("[$($MyInvocation.MyCommand)] Using CloudFront source for preview " +
                            "version (from string): $semverString")
                        ...
                        if ($Version -match '^\d+\.\*$') {
                            $PSCmdlet.ThrowTerminatingError(
                                [System.Management.Automation.ErrorRecord]::new(
                                    ([System.ArgumentException]"Wildcard version patterns should be resolved before calling Get-AWSToolsZip. Version: $Version"),
                                    'UnresolvedWildcardVersion',
            ...
            ScriptBlock = {
                ...
                if (-not $response.Success) {
                    ...
                    $customMessages = @{
                        ...
                        403 = "The version specified is not available online. Please try another version."
